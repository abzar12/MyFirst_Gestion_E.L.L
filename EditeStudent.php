...
if($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['UpdateStudent'])){
    ...
    $stmt = $conn->prepare("UPDATE Students SET Nom = :Nom, Prenom = :Prenom, DateNaissance = :Birthday, Country = :Country, TelephoneWhatsapp = :TelWhatsapp, TelephoneGhana = :TelGhana, Classe = :Classe, Formation = :Formation, Dure = :Dure WHERE ID= :ID");
    $stmt->bindParam(':ID',$IdStudent);
                        ...
                        <?php foreach ($result as $row): ?>
                            <tbody>
                                <tr>
                                    ...
                                    <td><input type="text" name="Classe" style="border: none; text-align:center; width:90px" value="<?= htmlspecialchars($row['Classe']); ?>"></input></td>
